...
Hopefully, you will find this template somewhat suitable to your need because we spent more than a day to gather everything.
...
The reason is that we use multiple fonts in this project. If you do not want to use multiple fonts then you have to modify the template a little bit by removing all font commands and usages. All the chapters, sections, subsections, and subsubsections use Tahoma font, so remove them as well (with simple search and replace).
============================================================================ <br>
...
- ``configurations.tex`` is the file that we put most of our packages and commands. We separate packages and commands according to (self-defined) categories like **pape setup**, **font setup**, etc. so that we can track thing easier but you can write them anywhere. What floats your boat.
